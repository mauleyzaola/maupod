...
type mediumR struct {
	MediaEvents MediaEventSlice `boil:"MediaEvents" json:"MediaEvents" toml:"MediaEvents" yaml:"MediaEvents"`
}
...
// MediaEvents retrieves all the media_event's MediaEvents with an executor.
func (o *Medium) MediaEvents(mods ...qm.QueryMod) mediaEventQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"media_event\".\"media_id\"=?", o.ID),
	)

	query := MediaEvents(queryMods...)
	queries.SetFrom(query.Query, "\"media_event\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"media_event\".*"})
	}

	return query
}

// LoadMediaEvents allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (mediumL) LoadMediaEvents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMedium interface{}, mods queries.Applicator) error {
	var slice []*Medium
	var object *Medium
	...
	query := NewQuery(
		qm.From(`media_event`),
		qm.WhereIn(`media_event.media_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load media_event")
	}

	var resultSlice []*MediaEvent
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice media_event")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on media_event")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for media_event")
	}

	if singular {
		object.R.MediaEvents = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &mediaEventR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.MediaID {
				local.R.MediaEvents = append(local.R.MediaEvents, foreign)
				if foreign.R == nil {
					foreign.R = &mediaEventR{}
				}
...
// AddMediaEvents adds the given related objects to the existing relationships
// of the medium, optionally inserting them as new records.
// Appends related to o.R.MediaEvents.
// Sets related.R.Medium appropriately.
func (o *Medium) AddMediaEvents(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MediaEvent) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.MediaEvents = append(o.R.MediaEvents, related...)
	}
