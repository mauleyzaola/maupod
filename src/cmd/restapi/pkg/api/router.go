...
func SetupRoutes(a *ApiServer, output io.Writer) http.Handler {
	...
	baseRouter.HandleFunc("/file/sync", chainGlueCors(a.FileSync)).Methods(http.MethodOptions, http.MethodPost)
	...
	baseRouter.HandleFunc("/playlists/{id}/items/{position}", chainGlueCors(a.PlaylistItemPut)).Methods(http.MethodOptions, http.MethodPut)
	...
	baseRouter.HandleFunc("/volume", chainGlueCors(a.VolumeChange)).Methods(http.MethodOptions, http.MethodPost)
